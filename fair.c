...
static void update_min_vruntime(struct cfs_rq *cfs_rq, int who)
{
    ...
    int flag = 0;
    u64 min_vruntime_copy = cfs_rq->min_vruntime;

    /*
    if (cfs_rq->curr && entity_is_task(cfs_rq->curr) 
            && (cpu == 4 || cpu == 6)) {
        struct task_struct* curr_p = task_of(cfs_rq->curr);
        if (strcmp(curr_p->comm, "ploop") == 0 || strcmp(curr_p->comm, "wloop") == 0) {
            printk("In UPdate_Min who: %d, cur_cpu: %d, cur_pid: %d, cur_name: %s, vrun: %llu, cfs_vrun: %llu\n",
                    who, smp_processor_id(), curr_p->pid, curr_p->comm, cfs_rq->curr->vruntime, cfs_rq->min_vruntime);
            flag = 1;
        }
    }
    ...
    * End of Mod */
	...
	if (cfs_rq->rb_leftmost) {
         ...
         *
        if (cfs_rq->curr && entity_is_task(cfs_rq->curr) 
                && entity_is_task(se) && (cpu == 4)) {
            struct task_struct* p = task_of(se);
            struct task_struct* curr_p = task_of(cfs_rq->curr);
            if (strcmp(p->comm, "ploop") == 0 || strcmp(p->comm, "wloop") == 0) {
                printk("In UPdate_Min who: %d, cur_cpu: %d, cur_pid: %d, cur_name: %s, pvrun: %llu, left_pid: %d, left_name: %s, left_vrun: %llu, updated_vrun: %llu, cfs_vrun: %llu\n",
                        who, smp_processor_id(), curr_p->pid, curr_p->comm, cfs_rq->curr->vruntime, p->pid, p->comm, se->vruntime, vruntime, cfs_rq->min_vruntime);
                flag = 1;
            }
        }
        * End of Mod */
...
static void __enqueue_entity(struct cfs_rq *cfs_rq, struct sched_entity *se)
{
    ...
    if (strcmp(p->comm, "ploop") == 0) {
        printk("In Enqueue_Entity_1 pid: %d, name: %s, jiffies: %llu, vruntime: %llu leftmost: %d\n", p->pid, p->comm, jiffies, se->vruntime, leftmost);
        flag = 1;
    }
    ...
    if (left && (flag == 1)) {
        ...
        lp = task_of(lse);
        printk("In Enqueue_entity_2 lpid: %d, lname: %s, jiffies: %llu, lvruntime: %llu \n", lp->pid, lp->comm, jiffies, lse->vruntime);
    }
    ...
    * end of mod */
    ...
    if (left && (flag == 1)) {
        lse = rb_entry(left, struct sched_entity, run_node);
        lp = task_of(lse);
        printk("In Enqueue_entity_3 lpid: %d, lname: %s, jiffies: %llu, lvruntime: %llu \n", lp->pid, lp->comm, jiffies, lse->vruntime);
    }
 ...
 */
static void update_curr(struct cfs_rq *cfs_rq, struct sched_entity* se, int who)
{
    ...
    if ((cpu == 2) && strcmp(p->comm, "ploop") == 0) {
        if (curr->counter < 15) {
            curr->arr_vruntime[curr->counter] = curr->vruntime;
            curr->cfs_vruntime[curr->counter] = cfs_rq->min_vruntime;
        } else {
           for (index = 0; index < 15; index++) {
                printk("index: %d the value is: %llu cfs: %llu\n", index, curr->arr_vruntime[index], curr->cfs_vruntime[index]);
           }

        }
        if (curr->counter == 10) {
            curr->ten_vruntime = curr->vruntime;
        }
        ...
        if (left) {
            lse = rb_entry(left, struct sched_entity, run_node);
            lp = task_of(lse);
            printk("In Update_curr lpid: %d, lname: %s, jiffies: %llu, lvruntime: %llu \n", lp->pid, lp->comm, jiffies, lse->vruntime);
        } else {
            printk("In Update_curr leftmost is NULL! The counter is: %d Parent: %d, name: %s, Real_Parent: %d, Name: %s\n", curr->counter, p->parent->pid, p->parent->comm, p->real_parent->pid, p->real_parent->comm);
        }
    }

    if ((curr != NULL) && entity_is_task(curr)) {
        p = task_of(curr);
        if ((strcmp(p->comm, "ploop") == 0) || strcmp(p->comm, "wloop") == 0) {
            printk("In UPdate_curr_1 who: %d, pid: %d, name: %s, vrun: %llu,  cur_cpu: %d, cfs_cpu: %d, jiffies: %llu, load: %d, utili: %d,  cfs_vrun: %llu num: %d\n",
            who, p->pid, p->comm, p->se.vruntime, cpu, cfs_rq->rq->cpu, jiffies, cfs_rq->curr->avg.load_avg_contrib, cfs_rq->curr->avg.utilization_avg_contrib, cfs_rq->min_vruntime, cfs_rq->nr_running);
        }
        flag = 1;
    }
    ...
    * End of Mod YZhao */
	...
	update_min_vruntime(cfs_rq, who);
    ...
    if (flag == 1 && ((strcmp(p->comm, "ploop") == 0) || strcmp(p->comm, "wloop") == 0)) {
        printk("In UPdate_curr_2 who: %d, cur_cpu: %d, cfs_cpu: %d, curr_pid: %d, name: %s, jiffies: %llu, cfs_load: %d, cfs_utiliz: %d, curr_vrun: %llu, cfs_vrun: %llu\n",
                who, cpu, cfs_rq->rq->cpu, p->pid, p->comm, jiffies, cfs_rq->runnable_load_avg, cfs_rq->utilization_load_avg, cfs_rq->curr->vruntime, cfs_rq->min_vruntime);
    }
...
{
	update_curr(cfs_rq_of(&rq->curr->se), 0, 0);
}
						  ...
						  int wakeup)
{
    ...
    if (entity_is_task(se)) {
		p = task_of(se);
        if ((strcmp(p->comm, "ploop") == 0) || strcmp(p->comm, "wloop") == 0) {
            printk("In Load_AVG cur_cpu: %d, cfs_cpu: %d, curr_pid: %d, name: %s, jiffies: %llu, cfs_load: %d, cfs_utiliz: %d, vrun: %llu, cfs_vrun: %llu\n",
                    cpu, cfs_rq->rq->cpu, p->pid, p->comm, jiffies, cfs_rq->runnable_load_avg, cfs_rq->utilization_load_avg, se->vruntime, cfs_rq->min_vruntime);
        }
    }
    * End of Mod YZhao*/
...
enqueue_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
    ...
    * End of Mod */
    ...
    struct task_struct* p = task_of(se);
    if ((strcmp(p->comm, "ploop") == 0)) {
        printk("In Enqueue_Entity pid: %d, name: %s, vruntime: %llu, cfs_vrun: %llu, curr: %d, curr_name: %s, vrun: %llu, jiffies: %llu\n",
                p->pid, p->comm, p->se.vruntime, cfs_rq->min_vruntime, task_of(cfs_rq->curr)->pid, task_of(cfs_rq->curr)->comm, cfs_rq->curr->vruntime, jiffies);
...
dequeue_entity(struct cfs_rq *cfs_rq, struct sched_entity *se, int flags)
{
    ...
    */
    ...
    if ((strcmp(p->comm, "ploop") == 0)) {
        printk("In Dequeue_Entity pid: %d, name: %s, vruntime: %llu, cfs_vrun: %llu, jiffies: %llu\n",
                p->pid, p->comm, p->se.vruntime, cfs_rq->min_vruntime, jiffies);
...
static void put_prev_entity(struct cfs_rq *cfs_rq, struct sched_entity *prev)
{
    ...
    int flag = 0;
    if (strcmp(p->comm, "ploop") == 0) {
        printk("In Put_Prev pid: %d, name: %s, prev_node: %d, jiffies: %llu, vruntime: %llu \n", p->pid, p->comm, prev->next_pid, jiffies, prev->vruntime);
        flag = 1;
    }
    ...
    if (left && (flag == 1)) {
        lse = rb_entry(left, struct sched_entity, run_node);
        lp = task_of(lse);
        printk("In Put_Prev lpid: %d, lname: %s, jiffies: %llu, lvruntime: %llu \n", lp->pid, lp->comm, jiffies, lse->vruntime);
    }
...
static int select_idle_sibling(struct task_struct *p, int target)
{
    ...
    cfs_rq = &rq->cfs;
    int cpu = 0;
    u64 max_diff_vruntime = 0;
    /* */
    
    /*
    if ((strcmp(p->comm, "ploop") == 0)) {
        printk("In Select_Idle_Sibling_1 pid: %d, name: %s, prev_cpu: %d, idle: %d, wake_cpu: %d, idle: %d\n",
                p->pid, p->comm, task_cpu(p), idle_cpu(task_cpu(p)), p->wake_cpu, idle_cpu(p->wake_cpu));
...
next:
     ...
     *
    if ((strcmp(p->comm, "ploop") == 0)) {
        printk("Before Select_Idle_Sibling pid: %d, name: %s, target: %d, idle: %d\n",
                p->pid, p->comm, target, idle_cpu(target));
...
static void rebalance_domains(struct rq *rq, enum cpu_idle_type idle)
{
     ...
     *
     *
    if ((strcmp(rq->curr->comm, "ploop") == 0 || strcmp(rq->curr->comm, "wloop") == 0)
            && (rq->cpu == 0) && (rq->cpu == 2) && (rq->cpu == 4) && (rq->cpu == 6)) {
        printk("Before IF cpu: %d, last_balance: %llu, jiffies: %llu, load: %d, nr: %d, curr_pid: %d\n",
                rq->cpu, sd->last_balance + interval, jiffies, rq->cfs.runnable_load_avg, rq->nr_running, rq->curr->pid);
    }
    * End of Mod YZhao */
	...
	for_each_domain(cpu, sd) {
        ...
        if ((strcmp(rq->curr->comm, "ploop") == 0 || strcmp(rq->curr->comm, "wloop") == 0)
                && ((rq->cpu == 0) || (rq->cpu == 2) || (rq->cpu == 4) || (rq->cpu == 6))) {
            printk("Before IF cpu: %d, next: %lu, last_balance: %llu, jiffies: %llu, load: %d, nr: %d, curr_pid: %d\n",
                    rq->cpu, rq->next_balance, sd->last_balance + interval, jiffies, rq->cfs.runnable_load_avg, rq->nr_running, rq->curr->pid);
        }
        * End of Mod YZhao */
...
out:
        ...
        if ((strcmp(rq->curr->comm, "ploop") == 0 || strcmp(rq->curr->comm, "wloop") == 0)
                && ((rq->cpu == 0) || (rq->cpu == 2) || (rq->cpu == 4) || (rq->cpu == 6))) {
            printk("After IF cpu: %d, last_balance: %llu, jiffies: %llu, load: %d, nr: %d, curr_pid: %d\n",
                    rq->cpu, sd->last_balance + interval, jiffies, rq->cfs.runnable_load_avg, rq->nr_running, rq->curr->pid);
        }
        if (rq->nr_running == 4) {
            if ((strcmp(rq->curr->comm, "ploop") == 0 || strcmp(rq->curr->comm, "wloop") == 0)
	...
	if (likely(update_next_balance))
        ...
        if ((strcmp(rq->curr->comm, "ploop") == 0 || strcmp(rq->curr->comm, "wloop") == 0)
                && ((rq->cpu == 0) || (rq->cpu == 2) || (rq->cpu == 4) || (rq->cpu == 6))) {
            printk("End Rebalance cpu: %d, next_balance: %llu, updated: %d, jiffies: %llu, load: %d, nr: %d, curr_pid: %d\n",
                    rq->cpu, rq->next_balance, update_next_balance, jiffies, rq->cfs.runnable_load_avg, rq->nr_running, rq->curr->pid);
        }
        * End of Mod YZhao */
...
static void task_fork_fair(struct task_struct *p)
{
    ...
    if (curr) {
        if (this_cpu == 2) {
            printk("In Task_Fork_1 cpu: %d, New_task: %d name: %s vrun: %llu, Curr_task %d, name: %s, vrun: %llu, jiffies: %llu, cfs: %llu\n", this_cpu, p->pid, p->comm, p->se.vruntime,
                    current->pid, current->comm, current->se.vruntime, jiffies, cfs_rq->min_vruntime);
        }
    }
    **/
     ...
     *
     *
        printk("In Task_Fork_2 cpu: %d, New_task: %d name: %s vrun: %llu, Curr_task %d, name: %s, vrun: %llu, jiffies: %llu, cfs: %llu\n", this_cpu, p->pid, p->comm, p->se.vruntime,
    if (this_cpu == 2) {
                current->pid, current->comm, current->se.vruntime, jiffies, cfs_rq->min_vruntime);
    }
    * End of Mod */
     ...
     *
    if (curr && (this_cpu == 2)) {
            printk("In Task_Fork_3 New_task: %d name: %s vrun: %llu, cpu: %d, Curr_task %d, name: %s, cpu: %d, vrun: %llu, jiffies: %llu, curr_pid: %d curr_name: %s, cfs: %llu\n", p->pid, p->comm, p->se.vruntime, task_cpu(p),
                    current->pid, current->comm, task_cpu(current), current->se.vruntime, jiffies, task_of(curr)->pid, task_of(curr)->comm, cfs_rq->min_vruntime);
    }
    se->vruntime_copy = 111111;
    se->cfs_vruntime_copy = 222222;
    se->counter = 0;
    * End of Mod YZhao */
